...
#############
{
	...
	DIR )
		...
		echo "`get_install_dir SA`/sun/jre_1.6.0_solaris_sun_sparc"
	    elif [ `plat_os` = "linux" ] && [ "${2:-}" = "32" ]; then
	 	echo "`get_install_dir SA`/sun/jre_1.6.0_linux_sun_i586"
	    elif [ `plat_os` = "linux" ] && [ "${2:-}" = "64" ]; then
	 	echo "`get_install_dir SA`/sun/jre_1.6.0_linux_sun_x64"
	    else
		echo "none"
	    fi
	    ;;
