...
{
    # The executables we're interested in are dbeng, dbsrv, dbisql, scjview, dbconsole, mlmon
	...
	DT_LIST="$DT_LIST sqlremote1201_pdf"
    fi
    if is_selected_option OPT_SAMON ; then
...
EOD
	...
	<Menu>
	    ...
	    <DirectoryDir>$XDG_DATA_HOME/desktop-directories</DirectoryDir>
            <Include>
		<Filename>${VENDORID}-Sybase Central.desktop</Filename>
		<Filename>${VENDORID}-Interactive SQL.desktop</Filename>
		<Filename>${VENDORID}-MobiLink Monitor.desktop</Filename>
		<Filename>${VENDORID}-SQL Anywhere Console.desktop</Filename>
		<Filename>${VENDORID}-SQL Anywhere Monitor-GNOME.desktop</Filename>
		<Filename>${VENDORID}-SQL Anywhere Monitor-KDE.desktop</Filename>
            </Include>
	</Menu>
	<Menu>
		...
		<Filename>${VENDORID}-SQL Remote Documentation.desktop</Filename>
...
writedesktopfile_dbisql()
{
    ...
    # Use the english description as the .desktop's filename
    RESKEY=\$EN_MSG_ICON_DBISQL
    DESC=`eval echo $RESKEY`
    DESC_UTF8=`convert_to_utf8 "$DESC"`

    EN_RESKEY_UTF8=\$EN_MSG_ICON_DBISQL
    EN_DESC_UTF8=`eval echo $EN_RESKEY_UTF8`

    DE_RESKEY_UTF8=\$DE_MSG_ICON_DBISQL
    DE_DESC_UTF8=`eval echo $DE_RESKEY_UTF8`

    JA_RESKEY_UTF8=\$JA_MSG_ICON_DBISQL
    JA_DESC_UTF8=`eval echo $JA_RESKEY_UTF8`

    ZH_RESKEY_UTF8=\$ZH_MSG_ICON_DBISQL
    ZH_DESC_UTF8=`eval echo $ZH_RESKEY_UTF8`
    
    FR_RESKEY_UTF8=\$FR_MSG_ICON_DBISQL
    FR_DESC_UTF8=`eval echo $FR_RESKEY_UTF8`
...
writedesktopfile_scjview()
{
    ...
    ICN=SybaseCentral.png
    DT_ENTRY_ICON_PATH="${INSTALLDIR_UTF8}/res/$ICN"

    # Use the english description as the .desktop's filename
    RESKEY=\$EN_MSG_ICON_SCENTRAL
    DESC=`eval echo $RESKEY`
    DESC_UTF8=`convert_to_utf8 "$DESC"`

    EN_RESKEY_UTF8=\$EN_MSG_ICON_SCENTRAL
    EN_DESC_UTF8=`eval echo $EN_RESKEY_UTF8`

    DE_RESKEY_UTF8=\$DE_MSG_ICON_SCENTRAL
    DE_DESC_UTF8=`eval echo $DE_RESKEY_UTF8`

    JA_RESKEY_UTF8=\$JA_MSG_ICON_SCENTRAL
    JA_DESC_UTF8=`eval echo $JA_RESKEY_UTF8`

    ZH_RESKEY_UTF8=\$ZH_MSG_ICON_SCENTRAL
    ZH_DESC_UTF8=`eval echo $ZH_RESKEY_UTF8`
    
    FR_RESKEY_UTF8=\$FR_MSG_ICON_SCENTRAL
    FR_DESC_UTF8=`eval echo $FR_RESKEY_UTF8`
...
writedesktopfile_dbconsole()
{
    ...
    # Use the english description as the .desktop's filename
    RESKEY=\$EN_MSG_ICON_DBCONSOLE
    DESC=`eval echo $RESKEY`
    DESC_UTF8=`convert_to_utf8 "$DESC"`

    EN_RESKEY_UTF8=\$EN_MSG_ICON_DBCONSOLE
    EN_DESC_UTF8=`eval echo $EN_RESKEY_UTF8`

    DE_RESKEY_UTF8=\$DE_MSG_ICON_DBCONSOLE
    DE_DESC_UTF8=`eval echo $DE_RESKEY_UTF8`

    JA_RESKEY_UTF8=\$JA_MSG_ICON_DBCONSOLE
    JA_DESC_UTF8=`eval echo $JA_RESKEY_UTF8`

    ZH_RESKEY_UTF8=\$ZH_MSG_ICON_DBCONSOLE
    ZH_DESC_UTF8=`eval echo $ZH_RESKEY_UTF8`
    
    FR_RESKEY_UTF8=\$FR_MSG_ICON_DBCONSOLE
    FR_DESC_UTF8=`eval echo $FR_RESKEY_UTF8`
...
writedesktopfile_mlmon()
{
    ...
    # Use the english description as the .desktop's filename
    RESKEY=\$EN_MSG_ICON_MLMON
    DESC=`eval echo $RESKEY`
    DESC_UTF8=`convert_to_utf8 "$DESC"`

    EN_RESKEY_UTF8=\$EN_MSG_ICON_MLMON
    EN_DESC_UTF8=`eval echo $EN_RESKEY_UTF8`

    DE_RESKEY_UTF8=\$DE_MSG_ICON_MLMON
    DE_DESC_UTF8=`eval echo $DE_RESKEY_UTF8`

    JA_RESKEY_UTF8=\$JA_MSG_ICON_MLMON
    JA_DESC_UTF8=`eval echo $JA_RESKEY_UTF8`

    ZH_RESKEY_UTF8=\$ZH_MSG_ICON_MLMON
    ZH_DESC_UTF8=`eval echo $ZH_RESKEY_UTF8`
    
    FR_RESKEY_UTF8=\$FR_MSG_ICON_MLMON
    FR_DESC_UTF8=`eval echo $FR_RESKEY_UTF8`
...
writedesktopfile_online_resources()
{
    ...
    # 
    LANGCODE=`get_lang_code`
    if [ -r "$INSTALLDIR/support/$LANGCODE/ianywhere.html" ]; then
	DOCFILE="$INSTALLDIR/support/$LANGCODE/ianywhere.html" 
    elif [ -r "$INSTALLDIR/support/en/ianywhere.html" ]; then
	DOCFILE="$INSTALLDIR/support/en/ianywhere.html" 
    else
	DOCFILE=
    fi
    if [ ! -r "$DOCFILE"  ]; then
...
writedesktopfile_dcx()
{
    ...
    DOCFILE="http://dcx.sybase.com/${VERSION}${LANGCODE}/contents.htm"
...
writedesktopfile_download_documentation()
{
    ...
    if [ "$LANGCODE" = "ja" ]; then
	DOCFILE="http://www.ianywhere.jp/developers/product_manuals/sqlanywhere/index.html"
    else
	DOCFILE="http://www.sybase.com/detail?id=1069195"
    fi
...
writedesktopfile_sqlremote1201_pdf()
{
    #
    # SQL Remote Including HTTP(S) Support
    # 
    DOCFILE="$INSTALLDIR/documentation/en/pdf/sqlremote1201.pdf"
    if [ ! -r "$DOCFILE"  ]; then
	return
    fi
    DOCFILE_UTF8=`convert_to_utf8 "$DOCFILE"`
    ...
    # Hard-code the .desktop's filename in this case
    RESKEY="SQL Remote Documentation"
    DESC=`eval echo $RESKEY`
    DESC_UTF8=`convert_to_utf8 "$DESC"`

    EN_RESKEY_UTF8=\$EN_MSG_ICON_SQLREMOTE_DOCUMENTATION
    EN_DESC_UTF8=`eval echo $EN_RESKEY_UTF8`

    DE_RESKEY_UTF8=\$DE_MSG_ICON_SQLREMOTE_DOCUMENTATION
    DE_DESC_UTF8=`eval echo $DE_RESKEY_UTF8`

    JA_RESKEY_UTF8=\$JA_MSG_ICON_SQLREMOTE_DOCUMENTATION
    JA_DESC_UTF8=`eval echo $JA_RESKEY_UTF8`

    ZH_RESKEY_UTF8=\$ZH_MSG_ICON_SQLREMOTE_DOCUMENTATION
    ZH_DESC_UTF8=`eval echo $ZH_RESKEY_UTF8`
    
    FR_RESKEY_UTF8=\$FR_MSG_ICON_SQLREMOTE_DOCUMENTATION
    FR_DESC_UTF8=`eval echo $FR_RESKEY_UTF8`
