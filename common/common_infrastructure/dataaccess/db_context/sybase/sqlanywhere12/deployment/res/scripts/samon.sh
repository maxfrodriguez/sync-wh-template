samon_set_discard_data()
########################
{
    __SAMON_DISCARD_DATA=$1
}
...
# $1: 1 if we should treat a migration failure as fatal, default 1
{
    ...
    JAVABINDIR=`get_install_dir ROOT`
    if [ "`plat_bitness`" = "32" ]; then
        BITNESSDIR="x86"
    else
        BITNESSDIR="x64"
    fi
    JAVABINDIR=$JAVABINDIR/sun/jre160_$BITNESSDIR/bin
    ...
    $JAVABINDIR/java -ea -Dsun.java2d.noddraw=true -Dsun.java2d.d3d=false -classpath "$SADIR/java/jodbc.jar:$SADIR/java/sajdbc.jar:$SADIR/java/jsyblib610.jar:$MEDIA_ROOT/res/migrator.jar" com.ianywhere.serverMonitor.migrator.MonitorMigrator $SWITCHES "`samon_backup_dir`/samonitor.db" "$SADIR/samonitor.db"
